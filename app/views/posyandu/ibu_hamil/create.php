    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="d-flex align-items-center mb-4 gap-3">
                ...
                <h4 class="fw-bold mb-0">Tambah Data Ibu Hamil</h4>
            </div>
            <div class="card border-0 shadow-sm">
                <div class="card-body p-4">
                    <form action="<?= url('posyandu/ibu-hamil/store') ?>" method="POST">
                        <?= csrfField() ?>
                        <div class="row g-3">
                            <div class="col-12">
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="bulan_kehamilan" class="form-select">
                                    <?php for ($i = 1; $i <= 9; $i++): ?>
                                        <option value="<?= $i ?>" <?= ($_POST['bulan_kehamilan'] ?? '1') == $i ? 'selected' : '' ?>>
                                            Bulan ke-<?= $i ?>
                                        </option>
                                    <?php endfor; ?>
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="tgl_perkiraan_lahir" class="form-control"
                                       value="<?= e($_POST['tgl_perkiraan_lahir'] ?? '') ?>">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Status Kesehatan</label>
                                <select name="status_kesehatan" class="form-select">
                                    <option value="normal" <?= ($_POST['status_kesehatan'] ?? 'normal') === 'normal' ? 'selected' : '' ?>>Normal</option>
                                    <option value="berisiko_tinggi" <?= ($_POST['status_kesehatan'] ?? '') === 'berisiko_tinggi' ? 'selected' : '' ?>>Berisiko Tinggi</option>
                                </select>
                            </div>
                            <div class="col-12">
